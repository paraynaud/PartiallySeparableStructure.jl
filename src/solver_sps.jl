module Solver_SPS
    ...
    function alloc_struct_algo(obj_Expr :: Expr, n :: Int, type=Float64 :: DataType )
        ...
        algo_struct = struct_algo(sps, B_k, B_k1, g_k, g_k1, y, x_k, x_k1, grad, (type)(0), (type)(0), Δ, η, η1, ϵ)
    ...
    function approx_quad(s_a :: struct_algo{T,Y}, x :: AbstractVector{Y}) where T where Y <: Number
        ...
        s_a.f_xk + g' * x  + PartiallySeparableStructure.product_matrix_sps(s_a.sps, s_a.B_k, x)' * x
    end
    ...
    function determine_xk1(s_a :: struct_algo{T,Y}) where T where Y <: Number
        ...
        cg_res = Krylov.cg(opB(s_a.B_k), - s_a.grad, radius = s_a.Δ) :: Tuple{Array{Y,1},Krylov.SimpleStats{Y}}
    ...
    function update_xk1!(s_a :: struct_algo{T,Y}) where T where Y <: Number
        ...
        if ratio > s_a.η
            s_a.x_k1 = next_x[1]
            s_k = s_a.x_k1 - s_a.x_k1 :: AbstractVector{Y}
            PartiallySeparableStructure.evaluate_SPS_gradient!(s_a.sps, s_a.x_k1, s_a.g_k1)
            PartiallySeparableStructure.minus_grad_vec!(s_a.g_k1, s_a.g_k, s_a.y)
    ...
    function solver_TR_SR1(obj_Expr :: Expr, n :: Int, x_init :: AbstractVector{Y}, type=Float64 :: DataType ) where Y <: Number
        ...
        s_a = alloc_struct_algo(obj_Expr :: Expr, n :: Int, type=Float64 :: DataType )
        ...
        cpt = 1
        while ( ( norm(s_a.g_k,2) > ϵ + n * sqrt(s_a.ϵ) )  &&  cpt < 50 )
            update_xk1!(s_a)
