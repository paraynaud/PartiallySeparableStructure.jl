module Solver_SPS
    ...
    function approx_quad(s_a :: struct_algo{T,Y}, x :: AbstractVector{Y}) where T where Y <: Number
        s_a.f_xk + s_a.grad_k' * x  +  1/2 *  PartiallySeparableStructure.product_matrix_sps(s_a.sps, s_a.B_k, x)' * x
    end


    function determine_xk1(s_a :: struct_algo{T,Y}) where T where Y <: Number
        n = s_a.sps.n_var
        opB(B :: PartiallySeparableStructure.Hess_matrix{Y}) = LinearOperators.LinearOperator(n, n, true, true, x -> PartiallySeparableStructure.product_matrix_sps(s_a.sps, B, x) )
...
# probleme l'approximation quadratiqe n'est pas du tout fiable, cg renvoie une valeur coérente mais cela ne colle pas avec la véritable fonction

    function compute_ratio(s_a :: struct_algo{T,Y}, s_k :: AbstractVector{Y}) where T where Y <: Number
        f_x_k = s_a.f_xk :: Y
        f_next_x = PartiallySeparableStructure.evaluate_SPS(s_a.sps, s_a.x_k + s_k) :: Y
        quadratic_approximation = approx_quad(s_a, s_k ) :: Y
        num = f_x_k - f_next_x :: Y
        den = f_x_k - quadratic_approximation :: Y
...
#copie
    ...
    function update_xk1!(s_a :: struct_algo{T,Y}) where T where Y <: Number
        (s_k, info) = determine_xk1(s_a) :: Tuple{Array{Y,1},Krylov.SimpleStats{Y}}
        (ratio, f_next_x) = compute_ratio(s_a, s_k :: Array{Y,1})
        println("norme de s_k: ", norm(s_k, 2), "  ratio: ", ratio)
        if ratio > s_a.η
            s_a.x_k1 = s_a.x_k + s_k
    ...
    function iterations_TR!(s_a :: struct_algo{T,Y}) where T where Y <: Number
        ...
        @printf "%3d \t%8.1e \t%7.1e \t%7.1e \n" cpt s_a.f_xk norm(s_a.grad_k,2) s_a.Δ

        while ( ( norm(s_a.grad_k) > s_a.ϵ + s_a.sps.n_var * s_a.ϵ^2 )  &&  cpt < cpt_max )
            update_xk1!(s_a)
            cpt = cpt + 1
            @printf "%3d \t%8.1e \t%7.1e \t%7.1e \n" cpt s_a.f_xk norm(s_a.grad_k,2) s_a.Δ
